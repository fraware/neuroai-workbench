...
def _supp(v: Any) -> str:
    return (_text(v) or ORIGINAL).upper()


def _identity(h: str, s: str) -> str:
    return f"HDE:{h}:{s}"


def _decision(v: Any) -> dict[str, Any]:
    code = (_text(v) or "").upper()
    sem = DECISION_MAP.get(code, "UNRESOLVED_DECISION_CODE")
    return {
        "decision_code": code or None,
        "decision_semantics": sem,
        "decision_code_recognized": code in DECISION_MAP,
        "decision_supports_hde_approval": code == "APPR",
        "decision_supports_reasonable_assurance_of_effectiveness": False,
        "decision_establishes_facility_irb_approval": False,
    }
...
def _normalize(raw: Mapping[str, Any], query_id: str) -> tuple[dict[str, Any] | None, str]:
    ...
    if scope != "HDE":
        return None, scope
    supp = _supp(raw.get("supplement_number"))
    dec = _decision(raw.get("decision_code"))
    record = {
        "record_kind": "NORMALIZED_OPENFDA_HDE_DECISION",
        "hde_number": hde,
        "supplement_number": supp,
        "record_identity": _identity(hde, supp),
        "record_role": "ORIGINAL_APPLICATION" if supp == ORIGINAL else "SUPPLEMENT",
        ...
        "decision_semantics": dec["decision_semantics"],
        "decision_code_recognized": dec["decision_code_recognized"],
        "decision_supports_hde_approval": dec["decision_supports_hde_approval"],
        "decision_supports_reasonable_assurance_of_effectiveness": False,
        "decision_establishes_facility_irb_approval": False,
    ...
    record["normalized_record_sha256"] = _sha(core)
    return record, scope
    ...
    *,
    ...
    for idx, raw_page in enumerate(pages, start=1):
        ...
        for raw in rows:
            ...
            if scope == "NON_H_OUT_OF_SCOPE":
                non_h += 1
                continue
            if scope != "HDE" or n is None:
                unresolved += 1
                continue
            identity = n["record_identity"]
            prior = by.get(identity)
            if prior is None:
                by[identity] = n
            else:
                a, b = dict(prior), dict(n)
                a.pop("query_memberships", None)
                b.pop("query_memberships", None)
                if a != b:
                    raise ValueError(f"Conflicting normalized HDE representations for {identity}")
                dup_count += 1
        reports.append(
            {
                "page_index": idx,
    ...
    state = "CONSISTENT" if reported is not None else "INCONSISTENT_ACROSS_PAGES"
    over = reported is not None and reported > MAX_DIRECT
    if reported is None:
        coverage = "DENOMINATOR_UNAVAILABLE"
    elif over:
        coverage = "OVER_LIMIT_SEARCH_AFTER_OR_PARTITION_REQUIRED"
    elif not seq:
        coverage = "INVALID_SEQUENCE"
    elif len(by) + non_h + unresolved == reported:
        coverage = "MATCH"
    else:
        coverage = "PARTIAL_OR_MISMATCH"
    records = []
    norms = []
    dups = []
    if not over:
        for identity in sorted(by):
            n = by[identity]
            duplicate = known.get(identity.upper())
            r = {
                "record_key": identity,
                "title": n.get("trade_name") or n.get("generic_name") or identity,
                "url": f"https://api.fda.gov/device/pma.json?search=pma_number:%22{n['hde_number']}%22",
                "publisher": "U.S. FDA",
                "source_class": "OFFICIAL_REGULATORY_HDE_RECORD",
                "suggested_source_id": f"SRC-OPENFDA-HDE-{n['hde_number']}-{n['supplement_number']}",
                "classification_hint": "DUPLICATE" if duplicate else "NEW",
                "decision_semantics": n["decision_semantics"],
            }
            if duplicate:
                r["duplicate_of_source_id"] = duplicate
                dups.append({"record_identity": identity, "source_id": duplicate})
            records.append(r)
            norms.append(n)
    cov = {
        "source_system": "OPENFDA_DEVICE_PMA_HDE_SUBSET",
        "query_id": query_id,
        "search_sha256": hashlib.sha256(search.encode()).hexdigest(),
        "supplied_page_count": len(pages),
        "returned_record_count": raw_count,
        "unique_composite_record_count": len(by),
        "reported_total_count": reported,
        "reported_total_count_state": state,
        "skip_sequence_valid": seq,
        "skip_coverage_state": coverage,
        "over_26000_limit": over,
        "search_after_or_partition_required": over,
        "out_of_scope_non_h_prefix_count": non_h,
        "known_controlled_duplicate_count": len(dups),
        "known_controlled_duplicates": dups,
        "new_candidate_count": len(records) - len(dups),
        "duplicate_representation_count": dup_count,
        "unresolved_hde_number_count": unresolved,
        "page_reports": reports,
        "decision_semantics_derived_only_from_exact_decision_code": True,
        "record_presence_is_hde_approval_claim": False,
        "hde_approval_is_reasonable_assurance_effectiveness_claim": False,
        "hde_approval_establishes_facility_irb_approval": False,
        ...
        "automatic_effectiveness_claim_creation_performed": False,
        "automatic_facility_irb_authorization_claim_creation_performed": False,
