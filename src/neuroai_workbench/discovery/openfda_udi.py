...
def _safe(v: Any) -> Any:
    ...
    if isinstance(v, Mapping):
        return {str(k): _safe(val) for k, val in sorted(v.items(), key=lambda p: str(p[0]))}
    return str(v)
...
def _identifier_rows(raw: Any) -> list[dict[str, Any]]:
    ...
    for item in raw:
        if not isinstance(item, Mapping):
            continue
        agency = _text(item.get("issuing_agency"))
        identifier = _text(item.get("id"))
        kind = _text(item.get("type"))
        if agency and identifier and kind:
            rows.append({"issuing_agency": agency, "id": identifier, "type": kind})
...
def _premarket_rows(raw: Any) -> list[dict[str, Any]]:
    ...
    for item in raw:
        ...
        row = {
            ...
            "submission_type": _text(
                item.get("submission_type") if "submission_type" in item else item.get("submissions_type")
            ),
        }
        if any(v is not None for v in row.values()):
            out.append(row)
    out.sort(
        key=lambda x: tuple(str(x.get(k) or "") for k in ("submission_type", "submission_number", "supplement_number"))
    )
    return out
...
def _product_codes(raw: Any) -> list[dict[str, Any]]:
    ...
    out.sort(key=lambda x: (x["code"], str(x.get("name") or "")))
    return out
...
def _normalize(raw: Mapping[str, Any], query_id: str) -> tuple[dict[str, Any] | None, str]:
    identifiers = _identifier_rows(raw.get("identifiers"))
    primary, count = _primary(identifiers)
    if count == 0:
        return None, "UNRESOLVED_PRIMARY_DI"
    if count > 1:
        return None, "MULTIPLE_PRIMARY_DI"
...
def project_search_pages(
    *,
    ...
    known_udi_sources: Mapping[str, str] | None = None,
) -> dict[str, Any]:
    ...
    for index, raw_page in enumerate(pages, start=1):
        ...
        if not isinstance(mr, Mapping) or not isinstance(rows, list) or not all(isinstance(r, dict) for r in rows):
            raise ValueError(f"page {index}: invalid openFDA UDI shape")
        ...
        for raw in rows:
            ...
            if state == "UNRESOLVED_PRIMARY_DI":
                unresolved += 1
                continue
            if state == "MULTIPLE_PRIMARY_DI":
                multiple += 1
                continue
            ...
            if prior is None:
                by_identity[identity] = n
            else:
                a, b = dict(prior), dict(n)
                a.pop("query_memberships", None)
                b.pop("query_memberships", None)
                if a != b:
                    raise ValueError(f"Conflicting normalized UDI representations for {identity}")
                duplicate_count += 1
        page_reports.append(
            {
                "page_index": index,
    ...
    if reported is None:
        ...
        coverage_state = "OVER_LIMIT_BULK_OR_PARTITION_REQUIRED"
    elif not sequence_valid:
        coverage_state = "INVALID_SEQUENCE"
    elif raw_count == reported:
    ...
    if not over:
        for identity in sorted(by_identity):
            n = by_identity[identity]
            dup = known.get(identity.upper())
            title = n.get("brand_name") or n.get("device_description") or identity
            r = {
                "record_key": identity,
                "title": title,
                "url": f"https://api.fda.gov/device/udi.json?search=identifiers.id:%22{n['primary_di']}%22",
                "publisher": "U.S. FDA",
                "source_class": "OFFICIAL_DEVICE_IDENTIFICATION_RECORD",
                "suggested_source_id": f"SRC-OPENFDA-UDI-{hashlib.sha256(identity.encode()).hexdigest()[:16].upper()}",
                "classification_hint": "DUPLICATE" if dup else "NEW",
            }
            if dup:
                r["duplicate_of_source_id"] = dup
                known_dups.append({"record_identity": identity, "source_id": dup})
            records.append(r)
            norms.append(n)
    coverage = {
        "source_system": "OPENFDA_DEVICE_UDI_GUDID",
        "query_id": query_id,
        "search_sha256": hashlib.sha256(search.encode()).hexdigest(),
        "supplied_page_count": len(pages),
        "returned_record_count": raw_count,
        "unique_primary_di_record_count": len(by_identity),
        ...
        "bulk_download_or_partition_required": over,
        ...
        "unresolved_primary_di_count": unresolved,
        "multiple_primary_di_count": multiple,
        "page_reports": page_reports,
        "automatic_device_or_system_entity_creation_performed": False,
        "automatic_company_entity_creation_performed": False,
        "automatic_di_relationship_creation_performed": False,
