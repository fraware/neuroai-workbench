...
def _validate_protocol(protocol: dict[str, Any]) -> None:
    ...
    _require(
        ...
        "discovery protocol is not in the frozen pre-acquisition state",
    )
    _require(
        protocol.get("evidence_cutoff") == "2026-08-20T00:00:00Z",
        "unexpected discovery evidence cutoff",
    )
    ...
    for family in families:
        ...
        _require(
            ...
            f"{family_id}: discovery_terms must be non-empty",
        )
        observed_ids.append(family_id)
        for term in terms:
            _require(
                isinstance(term, str) and term.strip(),
                f"{family_id}: discovery term must be non-empty",
            )
            normalized = term.strip().casefold()
            _require(
                normalized not in observed_terms,
                f"duplicate discovery term across frozen protocol: {term!r}",
            )
            observed_terms.add(normalized)
    _require(
        tuple(observed_ids) == EXPECTED_QUERY_FAMILIES,
        "frozen query-family identity or order changed",
    )

    inclusion = protocol.get("candidate_inclusion")
    _require(isinstance(inclusion, dict), "candidate_inclusion must be an object")
    _require(
        inclusion.get("relevance_adjudication_required") is True,
        "relevance adjudication must remain required",
    )
    _require(
        inclusion.get("automatic_canonical_inclusion") is False,
        "automatic canonical inclusion must remain disabled",
    )
    ...
    _require(
        dedup.get("fuzzy_title_author_matching") == "CANDIDATE_ONLY",
        "fuzzy matching must remain candidate-only",
    )
    _require(
        dedup.get("cross_provider_conflicts") == "PRESERVE_AND_ADJUDICATE",
        "provider conflicts must remain explicit",
    )
    ...
    _require(
        provider_policy.get("provider_absence_effect") == "NONE",
        "provider absence must have no canonical effect",
    )


def _validate_client_identity(provider: str, provider_contract: dict[str, Any]) -> None:
    identity = provider_contract.get("client_identity")
    _require(isinstance(identity, dict), f"{provider}: client_identity must be an object")
    _require(identity.get("access_class") == "PUBLIC", f"{provider}: access class changed")
    _require(
        identity.get("user_agent") == EXPECTED_FROZEN_USER_AGENT,
        f"{provider}: frozen client identity changed",
    )


def _validate_compilation(protocol: dict[str, Any], compilation: dict[str, Any]) -> None:
    _require(
        compilation.get("compilation_id") == EXPECTED_COMPILATION_ID,
        "unexpected query compilation id",
    )
    _require(
    ...
    _require(
        compilation.get("protocol_id") == protocol.get("protocol_id"),
        "query compilation protocol binding mismatch",
    )
    _require(
        compilation.get("provider_scope") == list(EXPECTED_PROVIDER_SCOPE),
        "query compilation provider scope changed",
    )

    supersedes = compilation.get("supersedes")
    _require(isinstance(supersedes, dict), "query compilation must record predecessor state")
    _require(
        supersedes.get("compilation_id") == "SCIENCE-QUERY-COMPILATION-V0.1",
        "unexpected compilation predecessor",
    )
    _require(
        supersedes.get("acquisition_state") == "NO_PROVIDER_ACQUISITION_EXECUTED",
        "predecessor acquisition state is not clean",
    )
    _require(
    ...
    priority_window = protocol.get("baseline_strategy", {}).get("priority_window")
    _require(isinstance(priority_window, dict), "protocol priority window is missing")
    _require(
        priority_window.get("from") == partitioning.get("from")
        and priority_window.get("through") == partitioning.get("through"),
        "query compilation partition does not match protocol priority window",
    )
    ...
    _require(
        isinstance(crossref, dict) and isinstance(europe_pmc, dict),
        "both provider contracts are required",
    )
    _validate_client_identity("CROSSREF", crossref)
    _validate_client_identity("EUROPE_PMC", europe_pmc)
    _require(
        crossref.get("source_universe_id") == "SU-SCI-CROSSREF",
        "Crossref source-universe binding mismatch",
    )
    _require(crossref.get("endpoint") == "https://api.crossref.org/works", "Crossref endpoint changed")
    _require(
        crossref.get("fixed_parameters") == {"rows": "1000", "select": "DOI,title,published"},
        "Crossref minimization changed",
    )
    _require(
        europe_pmc.get("source_universe_id") == "SU-SCI-EUROPEPMC",
    ...
    _require(
        minimization.get("state") == "PRE_ACQUISITION_MINIMIZED",
        "unexpected data-minimization state",
    )
    _require(
        minimization.get("crossref_selected_fields") == ["DOI", "title", "published"],
        "Crossref field declaration changed",
    )
    _require(
        minimization.get("europe_pmc_result_type") == "lite",
        "Europe PMC result type changed",
    )

    coverage = compilation.get("coverage_semantics")
    _require(isinstance(coverage, dict), "coverage_semantics must be an object")
    _require(
        coverage.get("query_unit_denominator_method") == "API_TOTAL",
        "query-unit denominator method changed",
    )
    _require(
    ...
    _require(
        coverage.get("deduplicated_union_is_derived") is True,
        "deduplicated union must remain derived",
    )
