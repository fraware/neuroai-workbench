...
Likewise, v1.6 source-check records have several fields that correspond to `Observation`, but they do not always encode every required native observation field such as the exact requested locator/retrieval-method contract. The proof therefore identifies reviewed field-level correspondences without claiming that the record is already materializable as a complete native `Observation`.
...
1. materialize complete native objects only where every required native field is supported without invention;
...
4. resolve entity subjects/objects deterministically before Event/Relationship/Assertion materialization;
5. run typed referential-integrity and mixed-precision temporal validation;
...
7. bind package compatibility, producer commit, runtime execution pin, graph schema generation, and S2 predecessor identity separately;
