...
- Production HTTP collection uses `PinnedSocketHttpTransport`, which connects only to DnsGuard-approved IP literals, preserves Host/SNI, does not self-follow redirects, verifies the connected INET peer against the selected pin and fail-closes on mismatch or an unverifiable peer, and records the verified connected address per response without mutable `last_connected_address` state.
...
## Online-first acquisition policy phase 1

Issue #279 adds a policy object without wiring it into network execution. The new threat surface is policy confusion: a stale, tampered, over-broad, or cross-source policy could be treated as permission for a network target it was not intended to cover, or a policy could be misrepresented as legal, institutional, evidentiary, assessment, release, or publication authority.

Phase 1 mitigates that class structurally. Policies are versioned and SHA-256 bound over canonical JSON; unknown fields fail closed; programme and source identifiers are bounded; each source carries its own execution-mode, exact-origin, and fallback rule; duplicate source rules and canonical-origin aliases are refused; approval/expiry timestamps are canonical and checked at request time; and URL canonicalization refuses wildcard hosts, user-info, trailing-dot hosts, IPv6 zones/percent escapes, malformed ports, policy queries/fragments, and non-root policy paths. Per-source origin binding prevents a multi-source policy from implicitly granting every source access to every origin in the policy.
