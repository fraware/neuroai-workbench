...
def test_transport_rejects_header_method_injection_and_host_override(http_request: HttpRequest, expected: str) -> None:
    transport = PinnedSocketHttpTransport(
        socket_factory=lambda target, timeout: pytest.fail("socket must not be opened")
    )
    with pytest.raises(CollectionFailureError) as exc:
        transport.send(http_request, connect_timeout=1.0, read_timeout=1.0)
