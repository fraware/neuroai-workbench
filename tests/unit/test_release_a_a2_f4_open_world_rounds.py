...
PACKET_RESOURCE = "RELEASE_A_A2_F4_OPEN_WORLD_ROUNDS_1_3_TRANCHE_1.v1.0.json"
PACKET_SHA256 = "abaff10977ad1f7f84b3311b6619d403184da4d94d805714913851c7d87ebfc3"
KNOWN_IDS = [
    "PRD-EMOTIV-EPOC-X",
    "PRD-FLOW-FL-100",
    "PRD-MODIUS-SPERO",
...
def test_f4_open_world_packet_is_content_bound_and_protocol_saturated() -> None:
    packet = _load_packet()
    universe = load_default_analysis_universe()
    f4_universe = load_default_open_world_query_universe("F4")
    ...
    assert packet["frame_id"] == "F4"
    assert packet["open_world_round_protocol_id"] == PROTOCOL_ID
    assert packet["open_world_round_protocol_sha256"] == PROTOCOL_SHA256
    assert packet["f4_query_universe_id"] == UNIVERSE_IDS["F4"] == f4_universe["universe_id"]
    assert packet["f4_query_universe_sha256"] == UNIVERSE_SHA256["F4"]
    ...
    assert open_world_frame_stop_state("F4", summaries) == "SATURATION_UNDER_DECLARED_PROTOCOL"


def test_f4_open_world_captures_and_runs_validate_under_frozen_contracts() -> None:
    packet = _load_packet()
    analysis = load_default_analysis_universe()
    frame = next(frame for frame in load_default_frame_register()["frames"] if frame["frame_id"] == "F4")
    f4_universe = load_default_open_world_query_universe("F4")
    allowed_seeds = {str(seed["query_or_seed_id"]) for seed in f4_universe["query_seeds"]}

    captures = cast(list[dict[str, Any]], packet["captures"])
    observations = {obs["observation_id"]: obs for obs in packet["observations"]}
    assert len(captures) == 78
    ...
    assert include_ids  # at least the A1 literature overlaps
...
def test_f4_open_world_round_accounting_matches_captures() -> None:
    packet = _load_packet()
    captures = cast(list[dict[str, Any]], packet["captures"])
...
def test_f4_open_world_packet_fails_closed_on_digest_or_estimator_drift() -> None:
    packet = _load_packet()
    bad = dict(packet)
    ...
    assert open_world_frame_stop_state("F4", packet["round_summaries"][:1]) == "CONTINUE"
