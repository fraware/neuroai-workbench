...
PACKET_RESOURCE = "RELEASE_A_A2_F6_OPEN_WORLD_ROUNDS_1_3_TRANCHE_1.v1.0.json"
PACKET_SHA256 = "95e13cc7dc896a0bad3f2d3862fda7475ff5e35291341f6bdbe5b9a0372e2e4c"
KNOWN_IDS = [
    "PRD-EMOTIV-EPOC-X",
    "PRD-FLOW-FL-100",
    "PRD-MODIUS-SPERO",
...
def test_f6_open_world_packet_is_content_bound_and_protocol_saturated() -> None:
    packet = _load_packet()
    universe = load_default_analysis_universe()
    f6_universe = load_default_open_world_query_universe("F6")
    ...
    assert packet["frame_id"] == "F6"
    assert packet["open_world_round_protocol_id"] == PROTOCOL_ID
    assert packet["open_world_round_protocol_sha256"] == PROTOCOL_SHA256
    assert packet["f6_query_universe_id"] == UNIVERSE_IDS["F6"] == f6_universe["universe_id"]
    assert packet["f6_query_universe_sha256"] == UNIVERSE_SHA256["F6"]
    ...
    assert open_world_frame_stop_state("F6", summaries) == "SATURATION_UNDER_DECLARED_PROTOCOL"


def test_f6_open_world_captures_and_runs_validate_under_frozen_contracts() -> None:
    packet = _load_packet()
    analysis = load_default_analysis_universe()
    frame = next(frame for frame in load_default_frame_register()["frames"] if frame["frame_id"] == "F6")
    f6_universe = load_default_open_world_query_universe("F6")
    allowed_seeds = {str(seed["query_or_seed_id"]) for seed in f6_universe["query_seeds"]}

    captures = cast(list[dict[str, Any]], packet["captures"])
    observations = {obs["observation_id"]: obs for obs in packet["observations"]}
    assert len(captures) == 99
...
def test_f6_open_world_round_accounting_matches_captures() -> None:
    packet = _load_packet()
    captures = cast(list[dict[str, Any]], packet["captures"])
...
def test_f6_open_world_packet_fails_closed_on_digest_or_estimator_drift() -> None:
    packet = _load_packet()
    bad = dict(packet)
    ...
    assert open_world_frame_stop_state("F6", packet["round_summaries"][:1]) == "CONTINUE"
