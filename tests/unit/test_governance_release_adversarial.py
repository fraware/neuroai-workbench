...
def _authority_claim() -> dict[str, str]:
    return {
        ...
        "accountability_state": release_mod.REAL_AUTHORITY_ACCOUNTABILITY_STATE,
        "execution_mode": release_mod.REAL_GOVERNANCE_EXECUTION_MODE,
...
def test_product_normalization_fail_closed_edges() -> None:
    ...
    normalized = release_mod._normalize_products(
        [
            {"product_id": "z", "sha256": D64_A},
            {"product_id": "a", "sha256": D64_B},
        ]
    )
    assert [item["product_id"] for item in normalized] == ["a", "z"]
...
def test_scope_candidate_digest_helper_fails_closed_on_ambiguity(
    ...
    monkeypatch.setattr(
        ...
        lambda _: [manifest, deepcopy(manifest)],
    )
    assert release_mod._scope_candidate_artifact_sha256(workspace, **kwargs) is None
...
def test_evaluation_reference_extraction_ignores_malformed_entries() -> None:
    ...
    assert dispositions[0]["condition_register_sha256"] == D64_C
...
def _patch_readiness_inputs(
    ...
    *,
    ...
    expected = release_mod._candidate_artifact_sha256(candidate) if scope_digest is None else scope_digest
    monkeypatch.setattr(
        release_mod,
        "_scope_candidate_artifact_sha256",
...
def test_readiness_builder_rejects_invalid_candidate_and_shape(
    ...
    for field in ("metadata", "predecessor_reference"):
...
def test_readiness_builder_exposes_all_blocker_classes(
    ...
    evaluation = _evaluation(
        ...
        track_results={
            "SECURITY": {"release_blocking_condition_ids": ["COND-1"]},
            "DOMAIN": "invalid",
        },
    )
...
def _patch_recording_prerequisites(
    ...
    *,
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: _ready_package(),
    )
    monkeypatch.setattr(
        release_mod,
...
def test_publication_rejects_invalid_store_and_wrong_prior(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace = _workspace(tmp_path)
...
def test_publication_rejects_readiness_candidate_duplicate_and_schema_drift(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace = _workspace(tmp_path)
    ...
    stale = deepcopy(auth)
    stale["readiness_package_reference"] = {
        "package_id": "other",
...
def test_store_verifier_detects_duplicate_event_ids_and_authorizations(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace = _workspace(tmp_path)
    ...
    monkeypatch.setattr(
        release_mod,
        "load_governance_release_decisions",
        lambda _: [record, duplicate],
    )
    monkeypatch.setattr(
        release_mod,
        "load_events",
        lambda _: [event, deepcopy(event)],
    )
    monkeypatch.setattr(release_mod, "verify_chain", lambda _: _valid_chain())
    joined = "\n".join(release_mod.verify_governance_release_decisions(workspace)["errors"])
...
def test_store_verifier_rejects_publication_prior_failures(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace = _workspace(tmp_path)
    ...
    monkeypatch.setattr(release_mod, "verify_chain", lambda _: _valid_chain())
    errors = release_mod.verify_governance_release_decisions(workspace)["errors"]
    assert any("publication lacks prior authorization" in error for error in errors)
    ...
    monkeypatch.setattr(release_mod, "load_events", lambda _: [_event_for(publication)])
    errors = release_mod.verify_governance_release_decisions(workspace)["errors"]
    assert any("prior authorization GOVREL-AUTH-MISSING is missing" in error for error in errors)


def test_store_verifier_rejects_prior_hash_candidate_and_package_drift(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace = _workspace(tmp_path)
    ...
    monkeypatch.setattr(
        release_mod,
        "load_events",
        lambda _: [_event_for(item) for item in records],
    )
    monkeypatch.setattr(release_mod, "verify_chain", lambda _: _valid_chain())
    joined = "\n".join(release_mod.verify_governance_release_decisions(workspace)["errors"])
    ...
    package.update(
        {
            "package_id": "GOVREADY-OTHER",
            "package_sha256": D64_D,
            "candidate_reference": {"different": True},
            "predecessor_reference": {"different": True},
            "governance_scope_reference": {"different": True},
            "reviewer_opinions": [{"different": True}],
            "owner_dispositions": [{"different": True}],
            "policy_evaluation_reference": {"different": True},
            "products": [{"different": True}],
            "withheld_claims_sha256": D64_D,
        }
    )
    monkeypatch.setattr(
        release_mod,
...
def test_schema_hash_and_candidate_artifact_helpers() -> None:
    assert release_mod._schema_errors({"schema_version": "0"})
    record = _authorization_record()
