    ...
    for track in sorted(REVIEW_TRACKS - (omit or set())):
        result[track] = [
            _opinion(
                ...
                scope,
                track=track,
                key=f"{track.lower()}-reviewer-a",
                organization=f"Synthetic {track} Organization A",
            ),
            _opinion(
                workspace,
                scope,
                track=track,
                key=f"{track.lower()}-reviewer-b",
                organization=f"Synthetic {track} Organization B",
            ),
        ]
    return result
...
def test_minority_objection_remains_blocking_after_owner_disposition(tmp_path: Path) -> None:
    ...
    objection = _opinion(
        ...
        scope,
        track="SECURITY",
        key="security-minority-objector",
        organization="Synthetic Security Organization C",
        state="OBJECT",
    )
...
def test_evidence_request_is_visible_and_blocking(tmp_path: Path) -> None:
    ...
    request = _opinion(
        ...
        scope,
        track="DOMAIN",
        key="domain-evidence-requester",
        organization="Synthetic Domain Organization C",
...
def test_unresolved_explicit_release_blocker_blocks_readiness(tmp_path: Path) -> None:
    workspace, scope = _workspace_and_scope(tmp_path)
    _populate_support(workspace, scope)
    conditional = _opinion(
        workspace,
        scope,
        track="METHODOLOGY",
        key="methodology-conditional",
        organization="Synthetic Methodology Organization C",
    ...
    disposition = record_governance_owner_disposition(
        ...
        conditions=[
            {
                "description": "Synthetic blocking condition.",
...
def test_declared_conflict_and_same_organization_fail_structural_independence(tmp_path: Path) -> None:
    ...
    _opinion(
        ...
        key="data-a",
        organization="Synthetic Shared Organization",
    )
    _opinion(
        workspace,
        scope,
        ...
        key="data-b",
        organization="Synthetic Shared Organization",
        conflict="CONFLICT_DECLARED: synthetic adversarial fixture",
    )
...
def test_unclassified_conflict_disclosure_does_not_satisfy_policy(tmp_path: Path) -> None:
    ...
    _opinion(
        ...
        track="DOMAIN",
        key="domain-b",
        organization="Synthetic Domain B",
        conflict="Free-form disclosure without the v1 machine marker",
    )
...
def test_non_human_accountability_claim_does_not_satisfy_human_reviewer_threshold(tmp_path: Path) -> None:
    ...
    for suffix, organization in (("a", "Synthetic Security A"), ("b", "Synthetic Security B")):
        _opinion(
            ...
            scope,
            track="SECURITY",
            key=f"security-{suffix}",
            organization=organization,
            accountability="SYNTHETIC_REHEARSAL_REVIEWER",
        )
