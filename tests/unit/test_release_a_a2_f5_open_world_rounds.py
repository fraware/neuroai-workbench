...
PACKET_RESOURCE = "RELEASE_A_A2_F5_OPEN_WORLD_ROUNDS_1_3_TRANCHE_1.v1.0.json"
PACKET_SHA256 = "0ce4e51a4f968ef452a1031d651e30119498fd4969563f91cadbccdeb2eaf3f5"
KNOWN_IDS = [
    "PRD-EMOTIV-EPOC-X",
    "PRD-FLOW-FL-100",
    "PRD-MODIUS-SPERO",
...
def test_f5_open_world_packet_is_content_bound_and_protocol_saturated() -> None:
    packet = _load_packet()
    universe = load_default_analysis_universe()
    f5_universe = load_default_open_world_query_universe("F5")
    ...
    assert packet["frame_id"] == "F5"
    assert packet["open_world_round_protocol_id"] == PROTOCOL_ID
    assert packet["open_world_round_protocol_sha256"] == PROTOCOL_SHA256
    assert packet["f5_query_universe_id"] == UNIVERSE_IDS["F5"] == f5_universe["universe_id"]
    assert packet["f5_query_universe_sha256"] == UNIVERSE_SHA256["F5"]
    ...
    assert open_world_frame_stop_state("F5", summaries) == "SATURATION_UNDER_DECLARED_PROTOCOL"


def test_f5_open_world_captures_and_runs_validate_under_frozen_contracts() -> None:
    packet = _load_packet()
    analysis = load_default_analysis_universe()
    frame = next(frame for frame in load_default_frame_register()["frames"] if frame["frame_id"] == "F5")
    f5_universe = load_default_open_world_query_universe("F5")
    allowed_seeds = {str(seed["query_or_seed_id"]) for seed in f5_universe["query_seeds"]}
...
def test_f5_open_world_round_accounting_matches_captures() -> None:
    packet = _load_packet()
    captures = cast(list[dict[str, Any]], packet["captures"])
...
def test_f5_open_world_packet_fails_closed_on_digest_or_estimator_drift() -> None:
    packet = _load_packet()
    bad = dict(packet)
    ...
    assert open_world_frame_stop_state("F5", packet["round_summaries"][:1]) == "CONTINUE"
