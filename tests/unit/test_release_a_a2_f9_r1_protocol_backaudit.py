...
PACKET_RESOURCE = "RELEASE_A_A2_F9_R1_PROTOCOL_BACKAUDIT_TRANCHE_6.v1.0.json"
LEDGER_RESOURCE = "RELEASE_A_F9_ACTOR_COMPLETION_LEDGER_002.v1.0.json"
PREDECESSOR_LEDGER_RESOURCE = "RELEASE_A_F9_ACTOR_COMPLETION_LEDGER_001.v1.0.json"
PRIOR_PACKET_RESOURCE = "RELEASE_A_A2_F9_BITBRAIN_PROTOCOL_TRANCHE_5.v1.0.json"
SUCCEEDED_PACKET_RESOURCE = "RELEASE_A_A2_F9_KNOWN_OVERLAP_TRANCHE_1.v1.0.json"
PACKET_SHA256 = "3212f66519f4af3ed645f4ada416c5da8d0782b04e4242da6a67f63b284ed5cf"
LEDGER_SHA256 = "2487d31b8236f057b35957ac73130012d8092ae712ecca0031df64d0f1f5ca17"
R1_ACTORS = {
    "ORG-0006",
    "ORG-0005",
    "ORG-0007",
...
def test_r1_protocol_backaudit_is_content_bound_successor_of_immutable_r1() -> None:
    ...
    assert set(packet["actors_protocol_completed_in_this_tranche"]) == R1_ACTORS
    ...
    assert set(succeeded["actors_accounted_in_this_tranche"]) == R1_ACTORS


def test_r1_protocol_backaudit_preserves_nonadmission_and_exact_capture_binding() -> None:
    packet = _load_json(PACKET_RESOURCE)
    universe = load_default_analysis_universe()
    ...
    assert len(packet["queries"]) == len(packet["observations"]) == len(captures) == 25
    assert (
    ...
    assert packet["source_recheck_registration"]["issue_comment_id"] == 5839906740

    outcomes = [str(capture["outcome"]) for capture in captures]
    assert "INCLUDE_RESOLVED" not in outcomes
    assert outcomes.count("UNRESOLVED_IDENTITY") == 12
    assert outcomes.count("ABSTAIN") == 13
    ...
    for capture in captures:
        ...
        assert query["organization_id"] in R1_ACTORS
...
def test_r1_protocol_backaudit_run_summary_and_completion_reproduce() -> None:
    packet = _load_json(PACKET_RESOURCE)
    universe = load_default_analysis_universe()
    frame_register = load_default_frame_register()
    f9_frame = next(frame for frame in frame_register["frames"] if frame["frame_id"] == "F9")
    ...
    assert run["round_id"] == "R6"
    assert run["capture_count"] == 25
    assert run["stop_state"] == "CONTINUE"
    assert {record["actor_organization_id"] for record in completions} == R1_ACTORS
...
def test_r1_successor_ledger_binds_predecessor_and_stays_continue() -> None:
    packet = _load_json(PACKET_RESOURCE)
    ...
    assert ledger["ledger_sequence"] == 2
    assert ledger["predecessor_ledger_id"] == predecessor["ledger_id"]
    ...
    assert ledger["completion_record_count"] == 7
    assert set(ledger["completed_actor_ids"]) == {"ORG-0003"} | R1_ACTORS
    assert ledger["actor_completion_records_remaining"] == 30
