...
def test_f9_second_tranche_is_content_bound_and_extends_prior_actor_accounting() -> None:
    ...
    assert prior_actors | tranche_actors <= frozen_actors
    assert len(tranche_actors) == packet["actors_accounted_count"] == 7
    assert len(prior_actors | tranche_actors) == packet["cumulative_actors_accounted_count"] == 13
    assert packet["actors_remaining_count"] == 24
    assert packet["execution_state"] == "PARTIAL_F9_EXECUTION"


def test_f9_second_tranche_preserves_unallocated_identity_and_world_time_boundary() -> None:
    packet = _load_json(PACKET_RESOURCE)
    universe = load_default_analysis_universe()
    ...
    assert outcomes == {"UNRESOLVED_IDENTITY", "ABSTAIN"}
...
def test_f9_second_tranche_run_and_summary_reproduce_without_new_identity_admission() -> None:
    packet = _load_json(PACKET_RESOURCE)
    universe = load_default_analysis_universe()
    frame_register = load_default_frame_register()
    f9_frame = next(frame for frame in frame_register["frames"] if frame["frame_id"] == "F9")
